...
from migrate.scanner_bridge import repos_from_scan, summary_from_scan
...
def cmd_check(args: argparse.Namespace) -> None:
    ...
    _load_rule_meta()
    stats = transform_directory(args.path, rules=_load_rules(), dry_run=True)
    s = _summary(stats)
    total = s["total"]

    if total == 0:
        print(f"OK  Nenhuma ocorrencia encontrada em '{args.path}'")
        return

    print(f"FAIL  {total} ocorrencia(s) encontrada(s) em '{args.path}'")
    ...
    print(f"      Execute: python -m migrate fix {args.path}")
    raise SystemExit(1)


def _repos_from_args(args: argparse.Namespace) -> list[str] | None:
    """Retorna lista de caminhos de repos se --from-scan foi usado, senão None."""
...
def cmd_report(args: argparse.Namespace) -> None:
    ...
    stats = transform_directory(args.path, rules=_load_rules(), dry_run=True)
    out_md = args.out or "migration_report.md"
...
def main() -> None:
    ...
    p_check.add_argument("path", help="Diretorio a verificar")
    p_check.set_defaults(func=cmd_check)

    p_scan = sub.add_parser("scan", help="Detecta ocorrencias sem alterar arquivos")
    p_scan.add_argument("path", help="Diretorio ou arquivo a escanear")
    ...
    p_fix.add_argument("path", help="Diretorio ou arquivo a transformar")
    ...
    p_report.add_argument("path", help="Diretorio a analisar")
